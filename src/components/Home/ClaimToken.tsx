...
export default function ClaimTokenMount() {
  ...
  const handleClaimToken = useCallback(async () => {
    const unityScore = window.getUnityScore?.() ?? 0;
    ...
    try {
      ...
      if (!privateKey) {
        setMessage("Private key missing");
        setIsLoading(false);
        return;
      }
  ...
  // 🔥 Attach function to window immediately
  useEffect(() => {
    // Attach the function to the window as soon as the component mounts
    window.handleClaimToken = handleClaimToken;
    console.log("handleClaimToken attached to window:", typeof handleClaimToken);
    ...
    return () => {
      ...
      delete window.handleClaimToken;
      console.log("handleClaimToken removed from window");
    };
  }, [handleClaimToken]);

  return null; // No UI — this is fine
}
